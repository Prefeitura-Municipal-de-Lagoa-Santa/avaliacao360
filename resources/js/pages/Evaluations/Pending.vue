...
const applyFilters = debounce(() => {
  ...
  router.get(route('evaluations.pending'), filters, {
    preserveState: true,
    replace: true,
  })
}, 300)
...
<template>
  ...
  <DashboardLayout pageTitle="Avaliações Pendentes (Todos)">
    ...
    <div v-if="props.pendingRequests.data.length === 0" class="text-center py-12">
      <icons.ClockIcon class="mx-auto h-12 w-12 text-gray-400 mb-4" />
      <h3 class="text-lg font-medium text-gray-900 mb-2">Nenhuma avaliação pendente encontrada</h3>
      <p class="text-gray-500 max-w-md mx-auto mb-4">
        {{ activeFiltersCount > 0 ? 'Tente ajustar os filtros para encontrar outras avaliações.' : 'Não há avaliações pendentes no momento.' }}
      </p>
      <button
        ...
        <div v-for="req in props.pendingRequests.data" :key="req.id" 
          ...
          <div class="flex justify-between items-start mb-3">
            <div class="flex-1 min-w-0">
              ...
              <p class="text-sm text-gray-500">por {{ req.avaliador }}</p>
            </div>
            <div class="flex items-center gap-2 ml-2">
              <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 capitalize">
                {{ req.type }}
              </span>
            </div>
          </div>
          ...
          <div class="space-y-2">
            <div class="flex items-center text-sm text-gray-500">
              <icons.FileTextIcon class="size-4 mr-2 text-gray-400" />
              {{ req.form_name }}
            </div>
            <div class="flex items-center text-sm text-gray-500">
        ...
        <template v-for="(link, key) in props.pendingRequests.links" :key="key">
          ...
          <Link
            ...
            :class="{
              ...
              'text-gray-500 bg-white': !link.active,
              'rounded-l-md': key === 0,
              'rounded-r-md': key === props.pendingRequests.links.length - 1
            }"
            :href="link.url"
